...
export default class HydrofoilResourceAccordion extends HydrofoilMultiResourceView {
   ...
   */
  // eslint-disable-next-line class-methods-use-this
  protected getIcon(model: HydraResource): string | undefined {
    return undefined
  }
   ...
   */
  protected renderAll() {
    const renderPanel = (model: any) => html`
      <paper-collapse-item
      ...
      >
        <div slot="header">
          ${this.areSame(model, this.root)
            ? ''
            : html`
                <paper-icon-button icon="close" @click="${this.close(model)}"></paper-icon-button>
              `}
        </div>
